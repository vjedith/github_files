...
class transaction;
  ...
  rand bit opr; // read or write operation (0 or 1)
  bit full,empty;
  bit rd,wr;
  bit [7:0] din;
  bit [7:0] dout;
...
class generator;
  ...
  task run(); 
    repeat (count) begin
      ...
      i++;
      mbx.put(tr);
      $display("[GEN] : Oper : %0d iteration : %0d", tr.opr, i);
      @(next);
    end 
    -> done;
  endtask
...
class scoreboard;
  ...
  bit [7:0] data_in[$];       // Array to store written data
